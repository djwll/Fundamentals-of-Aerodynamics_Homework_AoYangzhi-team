...
for i = 1:1:length(x)-1
    ...
    phi(end+1) = atan2(y_control(i),x_control(i));
...
for i = 1:1:length(x)-1
    for j = 1:1:length(y)-1
        if i ~= j
          A = -( x_control(i)-x(j) )*cos(phi(j))-(y_control(i)-y(j))*sin(phi(j));
          B = ( x_control(i)-x(j) )^2 + (y_control(i) - y(j))^2;
          C = -cos(phi(i)-phi(j));
          D = (x_control(i)-x(j))*cos(phi(i))+(y_control(i)-y(j))*sin(phi(i));
...
All_Metric(2*num+1,:)=0;
All_Metric(2*num+1,1)=1;
All_Metric(2*num+1,2*num+1)=1;
b = Q;
b(end)= pi/2;
Result = -inv(All_Metric)*cos(b)' ;
