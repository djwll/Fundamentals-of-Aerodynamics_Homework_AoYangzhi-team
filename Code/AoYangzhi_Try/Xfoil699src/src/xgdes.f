...
C--------------------------------------------------------
 ...
 1050  FORMAT(
     ...
     & /'   RERO     Rerotate (set chord line back from DERO)'
...
C---- save DERO derotation angle for RERO
       ARERO = ARAD
C
       CALL SCALC(XB,YB,SB,NB)
       CALL SEGSPL(XB,XBP,SB,NB)
...
C
C--------------------------------------------------------
      ELSEIF(COMAND.EQ.'RERO') THEN
       IF(ARERO.NE.0.0) THEN
        CALL ROTATE(XB,YB,NB,-ARERO)
        WRITE(*,1080) -ARERO / DTOR
 1082   FORMAT(/'Rerotating buffer airfoil by ',F8.3,' deg.')
C---- clear DERO derotation angle?
C       ARERO = 0.0
C
        CALL SCALC(XB,YB,SB,NB)
        CALL SEGSPL(XB,XBP,SB,NB)
...
C
      CALL NEWPEN(2)
      COLOVR = 'cyan'
      IF(NOVER.EQ.1) COLOVR = 'magenta'
      IF(NOVER.EQ.2) COLOVR = 'red'
      IF(NOVER.EQ.3) COLOVR = 'green'
      IF(NOVER.GE.4) COLOVR = 'blue'
      CALL PLTAIR(W1,W3,W2,W4,W5,NN, XOFF,XSF, YOFF,YSF,COLOVR)
      CALL PLNEWP(COLOVR)
C
