...
midIndX = floor(length(Cp_0)/2);                                     % Airfoil middle index for XFOIL data
midIndS = floor(length(Cp)/2);                                          % Airfoil middle index for SPM data
pXu = plot(X_0(1:midIndX),-Cp_0(1:midIndX),'b-','LineWidth',2);    % Plot Cp for upper surface of airfoil from XFOIL
pXl = plot(X_0(midIndX+1:end),-Cp_0(midIndX+1:end),'r-',...        % Plot Cp for lower surface of airfoil from XFOIL
                    'LineWidth',2);
