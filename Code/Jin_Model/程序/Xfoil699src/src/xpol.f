...
C---- HHY added XCpmin 
        NIPOL = NIPOL + 1
        IPOL(NIPOL) = IMCX
        write(*,*) 'plrset nipol,ipol() ',nipol,ipol(nipol)
       ENDIF
       IF(LHMOMP) THEN
        NIPOL = NIPOL + 1
        IPOL(NIPOL) = ICH
ccc        write(*,*) 'in PLRSET ich,ipol(ich) ',ich,ipol(ich)
       ENDIF
...
C---- HHY added XCpmin
      ...
      IF(LHMOMP) THEN
       NIPOL = NIPOL + 1
       IPOL(NIPOL) = ICH
ccc       write(*,*) 'in PLRINI ich,ipol(ich) ',ich,ipol(ich)
      ENDIF
C
      OPEN(LU,FILE=FNAME,STATUS='NEW',ERR=80)
      CALL POLWRIT(LU,' ',ERROR, .TRUE.,
     &             NAX, 0,-1, CPOL(1,1,IP), IPOL,NIPOL,
     &             REYNP1(IP),MACHP1(IP),ACRITP(IP),XSTRIPP(1,IP),
     &             NAMEPOL(IP),IRETYP(IP),IMATYP(IP),
     &             ISX,1,CPOLSD(1,1,1,IP), JPOL,NJPOL,
     &             'XFOIL',VERSION, .FALSE. )
...
C
        ...
        CPOL(IA,IMC,IP)  = CPMN
C---- HHY added XCpmin 
        CPOL(IA,IMCX,IP) = XCPMN
        write(*,*) 'plradd ia,ip,cpol ',ia,ipn,cpol(ia,imcx,ip)
C
        WRITE(*,1100) IP
...
C---- HHY added XCpmin 
       ...
       IF(LHMOMP) THEN
        NIPOL = NIPOL + 1
        IPOL(NIPOL) = ICH
ccc        write(*,*) 'in PLRADD ich,ipol(ich) ',ich,ipol(ich)
       ENDIF
C
